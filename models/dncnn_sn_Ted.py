...
@register_model("dncnn_sn_Ted")
class DnCNN_SN_TED(nn.Module):
    def __init__(self, depth=17, n_channels=64, image_channels=2, bnorm_type='mean', snorm=True, realsnorm=False, kernel_size=3, padding=1, L=0.76, n_power_iterations=5,residual=True, bias = True):
        super(DnCNN_SN_TED, self).__init__()
...
#                 layers.append(ScalarMultiplyLayer(L_layer))
            ...
            layers.append(nn.ReLU(inplace=True))
        conv = nn.Conv2d(in_channels=n_channels, out_channels=image_channels, kernel_size=kernel_size, padding=padding, bias=False)
        if snorm:
            layers.append(nn.utils.spectral_norm(conv,n_power_iterations=n_power_iterations))
            layers.append(ScalarMultiplyLayer(L_layer))
#         elif realsnorm:
...
#             layers.append(ScalarMultiplyLayer(L_layer))
            ...
            layers.append(conv)
        self.DnCNN_SN_TED = nn.Sequential(*layers)
