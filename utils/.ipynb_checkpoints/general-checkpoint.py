...
def calc_psnr_non_DB_scale(test_image, target_image, max=1.):
    """Calculate PSNR of images."""
    mse = mse_loss(test_image, target_image)
    return (max / torch.sqrt(mse)).item()

def calc_rSNR_non_DB_scale(x_rec,x_gt):
    num = torch.sum(x_gt**2)
    den = torch.sum((x_gt - x_rec)**2)
